...
**构筑数据库与性能优化**
- 构筑管理改为直接读写 `data/builds.json`，构筑推荐也直接引用同一文件。
- `community_builds.json` 已迁移为 `builds.json` 的构筑管理列表格式，游戏内编辑会直接写回该文件。
- 优化推荐计算缓存：同一帧复用持有物品扫描结果和最佳构筑匹配结果，减少商店/事件文本刷新卡顿。

### v7.6.4 (2026-07-10)

**构筑推荐与事件评分**
- 挖掘行动按任意英雄 + 中立遗物物品池显示推荐，并优先匹配带品质后缀的事件数据。
- 启动时使用 `data/builds.json` 作为构筑数据库，游戏内构筑管理直接读写该文件。
- 推荐文本百分比改为按天数出现权重计算，固定品质事件按等权计算，避免只按数量造成误导。
### v7.6.3 (2026-07-10)
